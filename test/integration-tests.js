...
runner.test('Firebase service integration test', async () => {
    ...
    const loadedComments = await global.mockFirebase.loadComments('test-url-hash');
    runner.assertEqual(loadedComments.length, 1, 'Should load exactly one comment');
    runner.assertEqual(loadedComments[0].text, testComment.text, 'Comment text should match');
});
...
runner.test('Comment validation integration', () => {
    ...
    function validateComment(comment) {
        ...
        if (!comment.timestamp || typeof comment.timestamp !== 'number') return false;
        return true;
    }
    
    runner.assert(validateComment(validComment), 'Valid comment should pass validation');
    runner.assert(!validateComment(invalidCommentEmpty), 'Empty comment should fail validation');
    runner.assert(!validateComment(invalidCommentLong), 'Long comment should fail validation');
});
...
runner.test('URL hash generation consistency', () => {
    ...
    testUrls.forEach(url => {
        ...
        runner.assertEqual(hash1, hash2, `Hash generation should be consistent for ${url}`);
...
runner.test('Logger integration with different log levels', () => {
    ...
    runner.assertEqual(mockLogs[0].level, 'info', 'First log should be info level');
    runner.assertEqual(mockLogs[1].level, 'error', 'Second log should be error level');
    runner.assertEqual(mockLogs[2].level, 'warning', 'Third log should be warning level');
});
...
runner.test('Firebase rules security validation', () => {
    ...
    runner.assert(commentRules.$urlHash['.read'] === true, 'Comments should be readable by all');
    runner.assert(commentRules.$urlHash['.write'] === 'auth != null', 'Comments should require authentication to write');
    
    // Test user rules security  
    const userRules = rules.rules.users;
    runner.assert(userRules.$userId['.read'] === 'auth != null && auth.uid == $userId', 'Users should only read their own data');
    runner.assert(userRules.$userId['.write'] === 'auth != null && auth.uid == $userId', 'Users should only write their own data');
    
    // Test validation rules exist
    runner.assert(commentRules.$urlHash.text['.validate'], 'Comment text should have validation');
    runner.assert(commentRules.$urlHash.author['.validate'], 'Comment author should have validation');
});
...
runner.test('File structure integrity', () => {
    ...
    requiredFiles.forEach(file => {
        ...
        runner.assert(fs.existsSync(filePath), `Required file ${file} should exist`);
    });
    ...
    requiredDirs.forEach(dir => {
        ...
        runner.assert(fs.existsSync(dirPath), `Required directory ${dir} should exist`);
    });
});
