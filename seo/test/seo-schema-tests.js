...
const SEOSchemaTests = {
  ...
  testUniversalSchema() {
    ...
    const orgSchema = universalSchema.find(s => s['@type'] === 'Organization');
    ...
    if (!websiteSchema.name || !websiteSchema.url || !websiteSchema.potentialAction) {
  ...
  testGetAllSchemas() {
    ...
    if (allSchemas.length < 5) { // 3 universal + 2 page-specific
  ...
  testDocumentationSchema() {
    ...
    if (!docSchema.audience || docSchema.audience.audienceType !== 'Developers') {
...
if (typeof process !== 'undefined' && process.argv && process.argv[1] && process.argv[1].includes('seo-schema-tests.js')) {
  SEOSchemaTests.runAllTests().then(results => {
